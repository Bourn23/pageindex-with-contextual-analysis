...
def parse_markdown_to_page_list(md_path: str, model: str = None) -> Tuple[List[Tuple[str, int]], List[dict], List[str]]:
    ...
    for i, entry in enumerate(toc_list):
        ...
        # if it's the first header, grab everything from the start of the file
        if i == 0:
            start_line = 0
        else:
    ...
    page_list = [(entry['text'], entry['token_count']) for entry in toc_list]
...
def build_tree_from_toc(toc_list: List[dict], total_lines: int) -> List[dict]:
    ...
    def add_text_to_tree(nodes, toc_map):
        for node in nodes:
            ...
            # Add dummy node to make it compatible with integration.py format
            node['start_index'] = 1
            node['end_index'] = 1
