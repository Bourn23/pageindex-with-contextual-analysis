...
def process_single_section(section_data: Dict[str, Any]) -> SectionNode:
    ...
    for grp in group_indices:
        ...
        # Process sentences inside this group
        sentence_nodes = []
        for i, sent_text in enumerate(group_sents):
            
            # Keyword Extraction
            kw_prompt = f"Extract key scientific terms from: '{sent_text}'"
            kw_json = llm_call(kw_prompt, KeywordOutput.model_json_schema())
            
            try:
                kw_data = KeywordOutput.model_validate_json(kw_json).keywords
            except:
                kw_data = []
            ...
            verified_keywords = []
            for kw in kw_data:
                # Only add if the term actually exists in the source text (case-insensitive check)
